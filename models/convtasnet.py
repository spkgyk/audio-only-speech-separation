...
class Conv1D(nn.Module):
    ...
    def __init__(self, in_channels=256, out_channels=512, kernel_size=3, dilation=1, norm="gln", causal=False):
        ...
        self.dwconv = nn.Conv1d(
            out_channels, out_channels, kernel_size=kernel_size, groups=out_channels, padding=self.pad, dilation=dilation
        )
...
class Encoder(nn.Module):
    ...
    def __init__(self, kernel_size=2, out_channels=64):
        ...
        self.conv1d = nn.Conv1d(in_channels=1, out_channels=out_channels, kernel_size=kernel_size, stride=kernel_size // 2, groups=1)
...
class Separation_TasNet(nn.Module):
    ...
    def __init__(
        ...
        self.conv1d_list = self._Sequential(
            repeats, conv1d_block, in_channels=out_channels, out_channels=out_sp_channels, kernel_size=kernel_size, norm=norm, causal=causal
        )
        self.PReLu = nn.PReLU()
    ...
    def _Sequential_block(self, num_blocks, **block_kwargs):
        ...
        """
        Conv1D_lists = [Conv1D(**block_kwargs, dilation=(2**i)) for i in range(num_blocks)]
    ...
    def _Sequential(self, num_repeats, num_blocks, **block_kwargs):
        ...
        input:
              ...
              **block_kwargs: parameters of Conv1D_Block
        """
        repeats_lists = [self._Sequential_block(num_blocks, **block_kwargs) for i in range(num_repeats)]
...
class Conv_TasNet(nn.Module):
    ...
    def __init__(self, N=512, L=16, B=128, H=512, P=3, X=8, R=3, norm="gln", num_spks=2, activate="relu", causal=False):
        ...
        self.separation = Separation_TasNet(
            repeats=R,
            conv1d_block=X,
            in_channels=N,
            out_channels=B,
        ...
        self.decoder = Decoder(in_channels=N, out_channels=1, kernel_size=L, stride=L // 2)
        self.num_spks = num_spks
